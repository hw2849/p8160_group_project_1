...
```{r, coxed simulation}
sim_data = sim.survdata(N = 500, T = 100, xvars = 2, censor = .2,  num.data.frames = 1)
attributes(sim_data)

head(sim_data$data$X1)
head(sim_data$data$X2)
head(sim_data$data$failed)

logrank.test(sim_data$data$X1, sim_data$data$failed, sim_data$data$X2, rho = 0, gamma = 0)

logrank.test(sim_data$data$X1, sim_data$data$failed, sim_data$data$X2, rho = 1, gamma = 0)

logrank.test(sim_data$data$X1, sim_data$data$failed, sim_data$data$X2, rho = 0, gamma = 1)
```
...
nonproportional-test: observed and expected 不一致

repeat tests under the same setting 

scenario 1: prop - same distribution(log normal), treatment and control group
scenerio 2: nonprop - one from exp, the other one from weibull distribution
