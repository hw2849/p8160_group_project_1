...
```{r, proportional}
set.seed(8160)
sim_data = sim.survdata(N = 1000, T = 100, xvars = 1, censor = .2, num.data.frames = 1) # simulating data

head(sim_data$data$X)
head(sim_data$data$y) 
head(sim_data$data$failed)

plot(sim_data$data$X, sim_data$data$y)
...
# log-rank
logrank.test(sim_data$data$X, sim_data$data$failed, sim_data$data$y, rho = 0, gamma = 0)
